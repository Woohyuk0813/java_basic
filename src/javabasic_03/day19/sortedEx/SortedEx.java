...
//스트림 내에서 요소를 오름차순 정렬(자연순서) 또는 사용자 정의 비교기에 따라 정렬 할 수 있다.
public class SortedEx {
    public static void main(String[] args) {
        ...
        //4. 중간 연산 체이닝
        List<String> names2 = Arrays.asList("Alice", "Bobi", "Charlie", "Dan", "Eve", "David");
        //names2의 이름 중 "D" 시작하는 이름을 대문자로 모두 바꾸어, 자연정렬하여 출력하세요.

        names2.stream().filter(name -> name.startsWith("D"))
                .map(String::toUpperCase)
                .sorted()
                .forEach(System.out::println);

        List<String> names122 = names2.stream().sorted().toList();
        System.out.println(names122);
